...
- **Four structural bans** in `skills/pencil-design/SKILL.md` Anti-patterns section: glassmorphism by default, the hero-metric template, nested cards, and modal-as-first-thought. Existing visual bans (Inter as UI font, neon glow effects, pure black or white bound directly) keep their place; the additions cover structural patterns the model reaches for without thinking.
- **Five new reference files** under `skills/pencil-design/references/`:
  - **`brand.md`** carries the brand-register depth: the anti-reference exercise (naming what the brand explicitly is NOT), the seven currently-saturated aesthetic lanes to reflex-reject, the four colour strategies (restrained / committed / full palette / drenched), brand typography pairings with the exhausted-by-reflex list, hero anatomy beyond the SaaS template, choreographed motion, brand-specific bans, and Pencil-specific variable strategies per colour strategy.
  - **`product.md`** carries the product-register depth: information density modes (dense / balanced / airy) with worked examples, surface treatment (borders over shadows, hierarchy through spacing), semantic colour role weight (error / success / warning / info), typography for data (mono numerals, tabular figures, small caps on metadata), state motion (≤16ms feedback, 150–250ms transitions), the four empty-state types with copy shapes, microcopy specificity, product-specific bans, and Pencil-specific density tokens plus table anatomy.
  - **`typography.md`** is the fastest move out of AI default: why typography signals intent, the reflex-reject font list (display fonts to avoid, body fonts to avoid, pairings to avoid, fonts worth reaching for instead), pairing rules with combinations that work for brand and product, scale and hierarchy (weight contrast over size contrast, avoiding flat scales, tracking discipline), readability (measure, line height, the light-on-dark line-height bump), numerals (mono / tabular / proportional), and OpenType special moves (small caps, stylistic sets, all-caps with tracking).
  - **`color-and-contrast.md`** covers OKLCH theory (why OKLCH over HSL/HEX, perceptual uniformity, the chroma-collapses-at-extremes rule), the four palette strategies, tinted neutrals with a worked OKLCH example, the semantic role catalogue (`bg` / `bgSurface` / `border` / `text` / `textMuted` / `accent` / `error` / `success` / `warning` / `info`), light + dark mode parity (what changes, what stays the same, common dark-mode failures), the reflex-reject palette defaults by category, and Pencil-specific `set_variables` patterns plus a contrast verification protocol.
  - **`ux-writing.md`** treats copy as a design surface: button labels (outcome-named with worked refusal/replacement table), error message structure (what + why + how-to-fix) with tone calibration table, the four empty-state copy patterns, section heading rules, microcopy specificity, the AI cliché list at three severity levels with replacement strategy, and Pencil-specific guidance on `content` properties and placeholder-name discipline.
- **`HARNESSES.md`** at the repo root: a cross-platform capability matrix listing frontmatter field support per harness, native skill directories, variable substitution syntaxes, platform-specific quirks, and the criteria for when a new platform earns a dedicated manifest.
- **Eight more reference files** under `skills/pencil-design/references/` (reference depth phase 2):
  - **`layout.md`** covers grids and what they're for, the 4pt/8pt baseline, spatial rhythm strategies (tight-then-airy, crescendo, asymmetric, pulse), the squint test, register-specific spatial strategy, the working layout patterns (single-column long-form, sidebar+main, dashboard grid, split-screen, bento), per-breakpoint guidance, and layout anti-patterns.
  - **`motion-design.md`** carries the depth behind the motion bullets in brand.md, product.md, and SKILL.md: the three jobs motion does (spatial continuity, state confirmation, expressive personality), five easing curves with character, duration tables by interaction type, register-specific motion (brand choreography vs product functional motion), dark-mode adjustments, `prefers-reduced-motion`, and Pencil-specific motion tokens.
  - **`interaction-design.md`** sits alongside `states.md` as the *how-to-design-states* depth (states.md remains the taxonomy): state design philosophy, the state inventory with how to design each well, register-specific state recipes, focus management depth (focus order, traps, skip links, restoration), and the state composition matrix.
  - **`cognitive-load.md`** covers the three load types (intrinsic / extraneous / germane), attention budget per surface type, hierarchy as cognitive scaffolding, density-vs-familiarity mismatch, register implications, and the populated-state verification rule.
  - **`heuristics-scoring.md`** is the structured UX-pass that complements distinctiveness-checklist.md's taste pass: ten heuristics (Nielsen-adapted) with 1–5 scoring rubrics each, composite score interpretation, worked-example scoring table, and anti-patterns in heuristic scoring.
  - **`delight.md`** is the signature-moment layer on top of competent design: what delight is and isn't, when it earns its place vs when it's wrong, working delight moves (copy with character, personality in functional motion, signature moments, recovery warmth), and the discipline rules (one moment per surface, test at scale, calibrate to brand voice).
  - **`onboard.md`** covers the activation moment and the three onboarding shapes (just-start / guided-first-action / setup-flow), what onboarding should not do, components of strong onboarding, empty states by user-state (first-use vs returning-empty vs result-empty vs permission-empty), and sample-data discipline.
  - **`extract.md`** is the *how-to-promote* depth that complements the components-first discipline rule: the six-step extraction workflow, what to extract (tokens / components / patterns), common extractions (token sweep, button, card, empty-state, icon-set), Pencil-specific `reusable: true` and `ref` mechanics, and library lifecycle.
...
- `AGENTS.md` Version Bumping section now lists four manifest files (added `.claude-plugin/marketplace.json` and `.codex-plugin/plugin.json`), drops the stale reference to a `SKILL.md` version field that the frontmatter never carried, and clarifies that the plugin holds at `0.8.0` pre-release with no bumps without owner approval.
- `AGENTS.md` Plugin System Rules now document the marketplace.json requirement.
- `AGENTS.md` Repository Structure tree adds the five new reference files plus `HARNESSES.md`; Links section adds `HARNESSES.md`.
- **`references/distinctiveness-checklist.md` enhanced** with the two-tier AI slop test (Question 9: category-reflex first-order plus second-order checks) and an aesthetic-lane reference move added to Question 1's fix guidance (*"reads like a Stripe internal tool"* over *"dense data product"* alone). The worked example table at the bottom now scores against the 9-question rubric.
- `skills/pencil-design/SKILL.md` cross-references the new references at five points: the register section links to `brand.md` / `product.md`, the negative-space color bullet links to `color-and-contrast.md`, the negative-space typography bullet links to `typography.md`, the AI-copywriting-clichés anti-pattern links to `ux-writing.md`, and the Reference index lists all five new files plus the updated 9-question framing.
- `skills/pencil-design/SKILL.md` "Design intelligence: when to deviate" section gains eight new trigger entries for the phase-2 references (layout, motion, interaction states, cognitive load, heuristics scoring, delight, onboarding, extraction), each with the specific signals that should load the reference.
