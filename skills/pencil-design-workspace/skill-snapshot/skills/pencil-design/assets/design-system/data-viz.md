...
The agent reads this when designing or generating a chart, sparkline, or dashboard tile. Without rules here, AI defaults to rainbow palettes, gridlines as loud as the data, and wildly inconsistent chart types. Decisions below pick a calm, scannable default.

## Categorical palette

For up to 8 series. Each color works in light AND dark, holds 3:1 contrast against `$surface` and `$surfaceMuted`, and is distinguishable for the most common color-vision deficiencies.
...
- **Don't use `$primary` for every chart.** It defeats the purpose of "primary == this is important." Reserve `$primary` for the focal series; rotate other chart colors otherwise.
- **No raw `#hex` in chart code.** Use the variables.
- **Charts with > 8 series:** group, filter, or switch to a different visualization. Don't add `$chart-9, 10, 11`.
- **Color-blind verification:** at least one in 12 men is color-blind. Test the palette in a deuteranopia simulator before shipping.

## Sequential palette (heatmaps, density)

For ordered data (low → high). Single-hue, varying lightness/saturation.
...
For diverging data (negative ↔ positive), declare a separate diverging palette here when needed, anchored on a neutral middle.
...
| **Empty** | Centered, muted icon + "No data yet" + (optional) "Connect a source" CTA. Match `voice.md` empty-state rules. |
...
- Use a single accent color (`$accent`) for annotations — distinct from the chart series colors.
...
If a chart needs a color or style that doesn't fit, add it here first (with both light and dark values, palette role explained). Don't ship a one-off chart palette inside a single component.
