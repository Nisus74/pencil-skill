...
The agent reads this when placing a logo, choosing brand colors for a marketing surface, or applying brand voice to a hero. Without rules here, AI defaults to placing the wordmark wherever and stretching marks to fit — both quietly wrong.
...
| **Monochrome variant** | `<assets/logo-mono.svg>` | Newsprint, single-color reproduction, photo overlays where the brand color clashes. |
...
- **Primary brand color:** `$primary` (defined in `tokens.md`). The logo renders in this color by default.
...
- **Never** recolor the logo to match a section's accent color — the logo is constant.
...
- Dark surface (`$surface` dark), centered logo (full lockup, white variant), tagline below.
...
- Apple touch icon (180 × 180) sits on a colored background — give it `$primary` (or a brand surface color) as the background, not transparency, since iOS doesn't render a transparent favicon well on the home screen.
