...
Add a row for every new component as you add it. When a column is empty, the project has explicitly decided that state isn't applicable; document why in the notes column.
...
|-------|----------------|
| **Focus** | 2px outline `$focusRing` with 2px offset. *(Override if the brand specifies a thicker / coloured ring.)* |
...
| **Loading (button)** | Replace label with a 16px spinner in the button's foreground colour. Preserve width. |
...
- **`saas-apps/b2b/analytics-dashboard`**: hover state on table rows is `$surfaceMuted` background (not a colour shift). Loading states are skeleton bars matching the final shape, not spinners. Error states: terse + retry. *"Couldn't load events. Retry"*. No apology theatre.
- **`saas-apps/b2b/modern-pro-tool`** (Linear): focus rings are tighter, often the 2px outline doubles as the only visible focus indicator since hover and active states use background-fill changes. Optimistic updates everywhere; loading states rarely appear on commit-style actions because the UI commits before the server confirms.
- **`marketing-websites/conversion-focused-saas`**: hover affordances quick (120–160ms); product-screenshot carousels rotate without an explicit loading state.

**What generic looks like (don't ship this):** spinner overlays on every action, *"Oh no, something went wrong! 😞"* error messages, success confetti modals, focus rings disabled in CSS because *"they're ugly"*.
...
See [`voice.md`](voice.md) § Empty states for the full copy template, length rules, and the *lead with the next action, not the absence* principle. The archetype-keyed empty-state variants live there too.
