...
> *"Design a dashboard in pencil."* And you notice during step 2 that there's no `design-system/` folder, so you offer.
...
|--------|-------------|------------|
| `assets/design-system/mobile.md` | `design-system/mobile.md` | User opts in; OR `package.json` has `react-native` / `expo` / `flutter`; OR `pubspec.yaml` exists; OR `ios/` / `android/` / `Podfile` present; OR build target includes iOS, Android, or mobile-web. |
...
When in doubt, **prefer including** an optional file (its top-of-file header says "Delete this file if your project doesn't…"). A user can delete a file in 2 seconds; reconstructing one they didn't know existed takes much longer.

Use platform-appropriate file ops (Bash `cp`, the `Write` tool reading from skill assets). The skill assets path is resolvable via the platform's skill-directory variable (e.g. `${CLAUDE_SKILL_DIR}` in Claude Code).

## Step 4: Tell the user what's there and what to fill in

> *"Scaffolded `design-system/` with 11 core templates [+ N optional: list them]. The two to edit first are:*
...
>
> *The others have sensible defaults you can leave for now. They're plain markdown, designed to be human-edited. If any of the optional files don't apply, delete them; each one has a 'Delete this file if…' header at the top."*

## Step 5: Continue with the original task (if applicable)

If you scaffolded because the original task was "design X" and you noticed the missing folder, return to step 3 of the default workflow (load guidelines), now reading the templates you just placed.

**At this point, pick an archetype.** Before any `batch_design` call on the original design task, scan `assets/archetypes/README.md` and pick the archetype whose surface category matches the brief. State it in your plan: "I'm building this in `analytics-dashboard`." The scaffold step does not produce a visual design; the first design task that follows it does, and that task needs an archetype to avoid generic output.
...
Take the answer, scaffold there. **Edit each template's internal references** so they point at the right relative paths if needed (the README's pointer to `tokens.md` should remain a relative reference that works in any sibling location).
